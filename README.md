...
## MAX bot and onboarding

`bot/main.py` запускает webhook-сервер, встроенный в `maxapi`; отдельный
FastAPI-сервис не используется. Контейнер называется `bot` и стартует так:

```text
python -m bot.main
```

Онбординг сохраняет согласие на обработку данных, город, режим гостя или
анкету пользователя. Для анкеты обязательны имя, пол, возраст и хотя бы три интереса; описание и
фото можно пропустить. Интересы выбираются кнопками внутри разделов, а не
вводятся текстом. Города и интересы берутся из БД,
поэтому `catalog-worker` должен хотя бы один раз успешно завершить импорт.

```powershell
docker build -f Dockerfile.bot -t dvizhmax-bot .
```

Для рабочего webhook укажите `MAX_WEBHOOK_URL` с публичным HTTPS-адресом и
`MAX_WEBHOOK_SECRET`. При старте бот создаёт подписку на `bot_started`,
`message_created` и `message_callback`, если её ещё нет.
