...
`/demo` — отдельное демо-событие, которого нет в афише, и три анкеты по порядку: без общих
интересов, с общими интересами и уже лайкнувшая тебя (мэтч и обмен контактом).
«🔄 Сбросить демо» начинает сценарий заново.
