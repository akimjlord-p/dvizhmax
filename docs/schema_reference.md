...
- CHECK `profile_status != 'active' OR (name IS NOT NULL AND length(trim(name)) > 0 AND gender IS NOT NULL AND length(trim(gender)) > 0 AND age IS NOT NULL AND city_id IS NOT NULL)`
